...
public class SparkTestAuthors {
    public static SparkCounter counter ;
    private static final Log log = LogFactory.getLog(SparkTestAuthors.class);

    public static void main(String[] args) {
        final JavaSparkContext context = new JavaSparkContext(new SparkConf().setAppName("Deduplication").setMaster("local[*]"));

        final URL dataset = SparkTest.class.getResource("/eu/dnetlib/pace/authors.json");
        ...
        final DedupConfig config = DedupConfig.load(readFromClasspath("/eu/dnetlib/pace/authors.test.pace.conf"));
        ...
        JavaPairRDD<String, String> relationRDD = mapDocs.reduceByKey((a, b) -> a)    //the reduce is just to be sure that we haven't document with same id
